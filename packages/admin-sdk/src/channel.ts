...
/**
 * Resembles the options that are available on any CicadaMessageType.
 */
 ...
 * @internal
 */
export type MessageDataType<TYPE extends keyof CicadaMessageTypes> = Omit<CicadaMessageTypes[TYPE], 'responseType'>;

/**
 * This is the structure of the data which will be send with {@link send}
 * @internal
 */
export type CicadaMessageSendData<MESSAGE_TYPE extends keyof CicadaMessageTypes> = {
  _type: MESSAGE_TYPE,
 ...
 * @internal
 */
export type CicadaMessageResponseData<MESSAGE_TYPE extends keyof CicadaMessageTypes> = {
  _type: MESSAGE_TYPE,
  _response: CicadaMessageTypes[MESSAGE_TYPE]['responseType'] | null,
  _callbackId: string,
}
 ...
 */
export function send<MESSAGE_TYPE extends keyof CicadaMessageTypes>(
  type: MESSAGE_TYPE,
  data: MessageDataType<MESSAGE_TYPE> & BaseMessageOptions,
  _targetWindow?: Window,
  _origin?: string
): Promise<CicadaMessageTypes[MESSAGE_TYPE]['responseType'] | null> {
  ...
  const messageData: CicadaMessageSendData<MESSAGE_TYPE> = {
    _type: type,
    _data: sendData,
  ...
  let serializedData = serialize(messageData) as CicadaMessageSendData<MESSAGE_TYPE>;
  ...
  if (_origin) {
    ...
    if (validationErrors) {
      ...
      if ([
        ...
        }) as CicadaMessageSendData<MESSAGE_TYPE>;
      }
  ...
  return new Promise((resolve, reject) => {
    const callbackHandler = function(event: MessageEvent<string>):void {
      ...
      const deserializedResponseData = deserialize(shopwareResponseData, event) as CicadaMessageResponseData<MESSAGE_TYPE>;
...
export type HandleMethod<MESSAGE_TYPE extends keyof CicadaMessageTypes> = (
  data: MessageDataType<MESSAGE_TYPE> & BaseMessageOptions,
  additionalInformation: { _event_: MessageEvent<string> }
) => Promise<CicadaMessageTypes[MESSAGE_TYPE]['responseType']> | CicadaMessageTypes[MESSAGE_TYPE]['responseType'];
 ...
 */
export function handle<MESSAGE_TYPE extends keyof CicadaMessageTypes>
(
  type: MESSAGE_TYPE,
  ...
  : () => void
{
  const handleListener = async function(event: MessageEvent<string>): Promise<void> {
    ...
    // Deserialize methods etc. so that they are callable in JS
    const deserializedMessageData = deserialize(shopwareMessageData, event) as CicadaMessageSendData<MESSAGE_TYPE>;
    ...
    const responseMessage: CicadaMessageResponseData<MESSAGE_TYPE> = {
    ...
    const serializedResponseMessage = ((): CicadaMessageResponseData<MESSAGE_TYPE> => {
      let serializedMessage = serialize(responseMessage) as CicadaMessageResponseData<MESSAGE_TYPE>;
      ...
      if (validationErrors) {
        ...
        serializedMessage = serialize(serializedMessage) as CicadaMessageResponseData<MESSAGE_TYPE>;
      }
...
export function publish<MESSAGE_TYPE extends keyof CicadaMessageTypes>(
  type: MESSAGE_TYPE,
  data: CicadaMessageTypes[MESSAGE_TYPE]['responseType'],
  sources: {
    source: Window,
    origin: string,
    sdkVersion: string | undefined,
...
export function subscribe<MESSAGE_TYPE extends keyof CicadaMessageTypes>(
  type: MESSAGE_TYPE,
  method: (data: CicadaMessageTypes[MESSAGE_TYPE]['responseType']) => void | Promise<unknown>
): () => void {
...
export function createSender<MESSAGE_TYPE extends keyof CicadaMessageTypes>
(messageType: MESSAGE_TYPE, baseMessageOptions: MessageDataType<MESSAGE_TYPE>)
:(messageOptions?: MessageDataType<MESSAGE_TYPE> & BaseMessageOptions) => Promise<CicadaMessageTypes[MESSAGE_TYPE]['responseType']>

// SENDER WITH PARTIAL ARGUMENTS (ARGUMENTS DEFINED IN BASE OPTIONS ARE OMITTED)
export function createSender<MESSAGE_TYPE extends keyof CicadaMessageTypes, BASE_OPTIONS extends Partial<MessageDataType<MESSAGE_TYPE>>>
(messageType: MESSAGE_TYPE, baseMessageOptions: BASE_OPTIONS)
:(messageOptions: Omit<MessageDataType<MESSAGE_TYPE>, keyof BASE_OPTIONS> & BaseMessageOptions) => Promise<CicadaMessageTypes[MESSAGE_TYPE]['responseType']>
 ...
 */
export function createHandler<MESSAGE_TYPE extends keyof CicadaMessageTypes>(messageType: MESSAGE_TYPE) {
  // eslint-disable-next-line @typescript-eslint/explicit-function-return-type
 ...
 * defined and can be hidden.
 */
export function createSubscriber<MESSAGE_TYPE extends keyof CicadaMessageTypes>(messageType: MESSAGE_TYPE) {
  // eslint-disable-next-line @typescript-eslint/explicit-function-return-type
  return (method: (data: CicadaMessageTypes[MESSAGE_TYPE]['responseType']) => void | Promise<unknown>, id?: string) => {
    if (!id) {
      return subscribe(messageType, method);
    }

    const wrapper = (data: CicadaMessageTypes[MESSAGE_TYPE]['responseType']): void => {
      if (data.id === id) {
        void method(data);
      }
    };
 ...
 */
function isMessageData<MESSAGE_TYPE extends keyof CicadaMessageTypes>(eventData: unknown): eventData is CicadaMessageSendData<MESSAGE_TYPE> {
  const shopwareMessageData = eventData as CicadaMessageSendData<MESSAGE_TYPE>;
...
// CicadaMessageTypes[MESSAGE_TYPE]['responseType']
function isMessageResponseData<MESSAGE_TYPE extends keyof CicadaMessageTypes>(eventData: unknown): eventData is CicadaMessageResponseData<MESSAGE_TYPE> {
  const shopwareMessageData = eventData as CicadaMessageResponseData<MESSAGE_TYPE>;
